...
def _remove_user(uid):
    """
    Remove user with specified user id from all queues.
...
def leave(message):
    ...
    users.pop(uid)
    # remove from auxiliary dictionaries
    ...
    if re.fullmatch(r'/remove( @\w+)+', message['text'].replace('\n', '')):
        ...
        if len(fail_verification) == 0:
            ...
            for alias in aliases:
                # get user id of user with this alias
                uid_ = uid_dict[alias]
...
def remove_queue(message):
    ...
    if re.fullmatch(r'/remove_queue \w+', message['text'].replace('\n', '')):
        ...
        if title in queues.keys():
            ...
            # remove all references to queue in users list
    ...
    if re.fullmatch(r'/current_user \w+', message['text'].replace('\n', '')):
        ...
        if title in queues.keys():
            ...
            if len(queues[title].queue) != 0:
                ...
                uid_current = queues[title].queue[current][0]
                message_current_user = '@' + alias_dict[
                    uid_current] + ', it is now your turn in queue <b>' + title + '</b>.'
            else:
